...
// - Thiha = MALE
// - Nilar = FEMALE
// - Male voice NEVER falls back to Google female
// - HF TTS always receives explicit voice information
// ============================================================
 ...
 *
 * IMPORTANT:
 * Google Translate TTS does NOT provide a reliable gender
 * selector through this endpoint.
 *
 * Therefore:
 *
 * google-male -> Edge Thiha male
 * google-female -> Google female
 *
 * Male must never use Google TTS fallback.
 */
function resolveVoiceMeta(voiceKey) {
  ...
  // GOOGLE MALE
  //
  // Google Translate TTS cannot reliably force male voice.
  // So this key is intentionally mapped to Thiha Edge voice.
  // ----------------------------------------------------------
  ...
  if (
    key === "google-female" ||
    key === "google-my-female"
  ) {
    return {
      gender: "female",
      isEdge: false,
      voiceName: "google-female",
      rate: "+0%",
      pitch: "+0Hz",
      label: "Google (မ)"
    };
...
async function startHFJob(text, meta) {
  ...
  /**
   * IMPORTANT:
   * Always send all four arguments.
   *
   * DO NOT fall back to:
   * data: [text]
   *
   * because the HF Space default voice may be Nilar/Female.
   */
  ...
  if (!res || !res.ok) {
    ...
    throw new Error(
      `HF TTS POST failed ${errorText.slice(0, 300)}`
    );
  }
  ...
  throw new Error(
    "HF TTS did not return event_id"
  );
}
...
async function waitForHFResult(eventId) {
  ...
  throw new Error(
    "HF TTS returned no audio result"
  );
}
...
async function resolveHFFileToBase64(audioValue) {
  ...
  if (
    ...
    if (!res.ok) {
      throw new Error(
        `HF audio download HTTP ${res.status}`
      );
    }

    return Buffer
      .from(await res.arrayBuffer())
      .toString("base64");
  }
  ...
  if (
    ...
    if (!res.ok) {
      throw new Error(
        `HF file download HTTP ${res.status}`
      );
    }
  ...
  throw new Error(
    "Unsupported HF audio result format"
  );
}
...
async function fetchEdgeTTSDirect(text, meta) {
  ...
  for (const chunk of chunks) {
    /**
     * meta.voiceName is explicitly passed here.
     *
     * Thiha:
     * my-MM-ThihaNeural
     *
    ...
    const job = await startHFJob(
      chunk,
      meta
    );

    const result =
      await waitForHFResult(job);

    const base64 =
      await resolveHFFileToBase64(result);
...
async function fetchAudioSafe(text, voiceKey) {
  ...
  if (
    ...
    try {
      ...
      errors.push(
        `Google TTS: ${err.message}`
      );
    }

    // Google female failed.
    // Try Edge Nilar instead.
    try {
      const fallbackMeta = {
        gender: "female",
        isEdge: true,
      ...
      return {
        audioBase64: audio,
        provider: "Edge-TTS",
        voice: fallbackMeta.label,
        gender: "female"
      };
    } catch (err) {
      errors.push(
        `Edge Female Fallback: ${err.message}`
      );
    }

    throw new Error(
      `မအသံ TTS ဝန်ဆောင်မှု မအောင်မြင်ပါ။ ${errors.join(" | ")}`
    );
  }
  ...
  if (meta.isEdge) {
    try {
      ...
      errors.push(
        `Edge-TTS (${meta.label}): ${err.message}`
      );
    }
  }
  ...
  // IMPORTANT:
  //
  // MALE VOICE MUST NOT FALL BACK TO GOOGLE TTS.
  //
  // Google Translate TTS endpoint cannot reliably force
  // Burmese male voice. If we use it here, the result can
  // become the default female voice.
  // ==========================================================

  if (meta.gender === "male") {
    throw new Error(
      `ကျားအသံ "${meta.label}" မအောင်မြင်ပါ။ ` +
      `HF Edge-TTS ကို စစ်ဆေးပါ။ ` +
      `Male voice အတွက် Google Female fallback ကို ` +
      `တမင်မသုံးထားပါ။ ` +
      errors.join(" | ")
    );
  }

  // Safety fallback for any unknown female route.
  try {
    const audio =
      await fetchGoogleTTS(script);
...
`.trim();
  ...
  for (let i = 0; i < count; i++) {
    prompts.push(
      rawLines[i] ||
      `Cinematic photorealistic movie scene ${
        i + 1
      }, natural environment, realistic characters, dramatic cinematic lighting, detailed composition, realistic film still, 16:9, no text, no watermark`
    );
  }
...
module.exports = async function handler(
  ...
  try {
    ...
    // ========================================================
    // TRANSLATE SCRIPT TO IMAGE PROMPTS
    // ========================================================
